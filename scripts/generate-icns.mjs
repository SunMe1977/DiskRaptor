...
async function main() {
  ...
  for (const icon of ICON_TYPES) {
    ...
    const resized = await img.clone().resize(targetSize, targetSize).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
    ...
    if (icon.type === 'ic04' || icon.type === 'ic05' || icon.type === 'ic06') {
      ...
      const data = convertToARGB(resized.data, resized.info.width, resized.info.height);
      iconEntries.push({
        type: icon.type,
        data: data,
