...
function killApp() { try { if (tp) { tp.kill('SIGKILL'); tp.kill(); } } catch {} }

async function connectBrowser(r) { r = r || 30; for (let i = 0; i < r; i++) { try { browser = await chromium.connectOverCDP('http://localhost:' + TEST_PORT, { timeout: 3000 }); return browser; } catch { await sleep(1000); } } throw new Error('Browser connect failed'); }

async function findPage() { for (let i = 0; i < 20; i++) { for (const ctx of browser.contexts()) for (const p of ctx.pages()) if (p.url().includes('tauri') || p.url().includes('about:blank')) return p; await sleep(1000); } throw new Error('No page found'); }

async function waitForTauri(p, r) { r = r || 30; for (let i = 0; i < r; i++) { const ok = await p.evaluate(() => typeof window.__TAURI__?.invoke === 'function').catch(() => false); if (ok) return; await sleep(1000); } throw new Error('Tauri bridge not ready'); }
...
async function run() {
  ...
  // Error tracking
  const pageErrors = [];
  page.on('pageerror', err => { console.log('  [PAGE_ERROR]', err.message); pageErrors.push(err.message); });

  // ═════════════════════════════════════════════════════════════════════
  //  1. invoke exists
  // ═════════════════════════════════════════════════════════════════════
  await test('invoke() is available', async () => {
    const ok = await page.evaluate(() => typeof window.__TAURI__.invoke === 'function');
  ...
  await test('pick_directory returns a path', async () => {
    const r = await page.evaluate(async () => {
      try { const p = await window.__TAURI__.invoke('pick_directory'); return { ok: true, path: p }; }
  ...
  //     We reuse this scan for all subsequent tests
  // ═════════════════════════════════════════════════════════════════════
  let scanId = null;
  ...
  await test('scan completes and returns result', async () => {
    ...
    for (let i = 0; i < 60; i++) {
      ...
      if (p && (!p.running || p.phase === 3)) { done = true; break; }
    }
    ...
    console.log('    files=' + result.files + ' dirs=' + result.dirs +
      ' size=' + result.totalSize + ' nodes=' + result.totalNodes +
      ' chunks=' + result.totalChunks);

    if (result.files === 0 && result.dirs === 0) throw new Error('Scan found 0 files');
  ...
  await test('chunk 0 has valid root node', async () => {
    ...
    const chunk = await page.evaluate(async (id) => {
      try {
        return await window.__TAURI__.invoke('get_chunk', { scanId: id, chunkIndex: 0 });
      } catch (e) { return { error: String(e) }; }
    }, scanId);

    if (chunk.error) throw new Error('get_chunk failed: ' + chunk.error);
    if (!chunk.nodes || chunk.nodes.length === 0) throw new Error('Chunk 0 empty');
    ...
    console.log('    root: ' + root.name + ' | size=' + root.size +
      ' files=' + root.file_count + ' type=' + root.node_type);
  })();
  ...
  await test('get_children returns root children', async () => {
    ...
    const children = await page.evaluate(async (id) => {
      try {
        return await window.__TAURI__.invoke('get_children', { scanId: id, nodeIndex: 0 });
      } catch (e) { return { error: String(e) }; }
    }, scanId);
    ...
    if (children.length > 0) {
      console.log('    first: ' + children[0].name + ' [' + children[0].size + ']');
    }
  })();
  ...
  await test('get_stats returns correct totals', async () => {
    ...
    if (typeof stats.total_files !== 'number') throw new Error('total_files missing');
    if (typeof stats.total_dirs !== 'number') throw new Error('total_dirs missing');
    console.log('    stats: files=' + stats.total_files + ' dirs=' + stats.total_dirs +
      ' size=' + stats.total_size + ' ms=' + stats.scan_time_ms);
  })();
  ...
  await test('release_scan works', async () => {
    ...
    await page.evaluate(async (id) => {
      try { await window.__TAURI__.invoke('release_scan', { scanId: id }); return { ok: true }; }
      catch (e) { return { ok: false, error: String(e) }; }
    }, scanId);
  })();
