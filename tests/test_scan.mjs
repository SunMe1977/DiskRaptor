...
function killAll() {
  try { execSync("taskkill /F /IM DiskRaptor.exe", { stdio: "ignore" }); } catch { /* best-effort: ignore */ }
  try { execSync("taskkill /F /IM QtWebEngineProcess.exe", { stdio: "ignore" }); } catch { /* best-effort: ignore */ }
}
