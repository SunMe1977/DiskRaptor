/**
 * DiskRaptor Linux Duplicates Tool Test.
 * Usage: node test_duplicates_linux.mjs
 */
...
async function main() {
  ...
  // === Duplicates Tool Tests ===
  console.log("\n=== Duplicates Tool Tests ===\n");

  // Open tools menu and click duplicates
  await jsExpr(cdp, `document.getElementById('btn-tools').click(); 'menu-opened'`);
  await sleep(300);
  const dupClick = await jsExpr(cdp, `(function(){ const items=document.querySelectorAll('.tools-item'); for(const i of items){ if(i.getAttribute('data-action')==='duplicates'){ i.click(); return 'clicked'; }} return 'not-found'; })()`);
  assert("Duplicates menu item clickable", dupClick === "clicked");
  await sleep(500);

  // Check duplicates panel/results exists
  const dupPanel = await jsExpr(cdp, `
    (function() {
      const p = document.getElementById('duplicates-panel');
      if (p) return 'panel-found';
      const all = document.querySelectorAll('[id*="duplicate"], [class*="duplicate"]');
      return all.length > 0 ? 'results-present-' + all.length : 'no-panel';
    })()
  `);
  assert("Duplicates panel rendered", dupPanel !== "no-panel", `${dupPanel}`);

  // Check for duplicate group or result content
  const dupContent = await jsExpr(cdp, `
    (function() {
      const p = document.getElementById('duplicates-panel') || document.querySelector('[class*="duplicate"]');
      if (!p) return 'no-container';
      const text = p.textContent.trim().slice(0, 80);
      return text.length > 0 ? 'content: ' + text : 'empty-content';
    })()
  `);
  assert("Duplicates content present", dupContent !== "no-container" && !dupContent.includes("empty"), `${dupContent}`);

  // Check for group headers (typically "Duplicate Group N" or similar)
  const hasGroupHeaders = await jsExpr(cdp, `
    (function() {
      const h = document.querySelectorAll('[class*="group"], [class*="duplicate-group"]');
      return 'groups=' + h.length;
    })()
  `);
  assert("Duplicate groups check", hasGroupHeaders.startsWith("groups="), `${hasGroupHeaders}`);

  // Check Tauri invoke for duplicates
  const dupInvoke = await jsExpr(cdp, `
    (async () => {
      try {
        const r = await window.__TAURI__.invoke('find_duplicates', { path: ${JSON.stringify(SCAN_PATH)} });
        return 'ok-' + JSON.stringify(r).slice(0, 50);
      } catch(e) { return 'err-' + e.message.slice(0, 40); }
    })()
  `);
  assert("find_duplicates Tauri invoke", dupInvoke.startsWith("ok") || dupInvoke.startsWith("err"), `${dupInvoke}`);
